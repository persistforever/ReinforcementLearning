...
class MonteCarloControlOnPolicy(MonteCarloControl):
	...
	def train(self, iteration=100000):
		...
		if self.observe:
			self.env.close()
		end_time = time.time()
		print '%s consumes %i tries' % ('on-policy', self.n_try)
		self.log.append('%s consumes %i tries' % ('on-policy', self.n_try))
		print '%s consumes %.2f seconds' % ('on-policy', end_time-start_time)
		self.log.append('%s consumes %.2f seconds' % ('on-policy', end_time-start_time))
		# save trajectory
	...
	def _get_log_path(self, map_size, index):
		return '../experiments/monte_carlo_control/trajectory/on-policy' + \
...
for size in [15]: # map size must be odd
	...
	for i in range(20):
		...
		on_policy = MonteCarloControlOnPolicy(observe=False, save=False, \
