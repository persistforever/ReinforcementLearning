...
class MonteCarloControlOffPolicy(MonteCarloControl):
	...
	def train(self, iteration=100000):
		...
		for _ in xrange(iteration):
			...
			while gone:
				...
				actionid = self._sample_action(stateid, self.behavior_policy)
			...
			self.behavior_policy = self._soft_policy_iteration()
			# exit judge
			if iteration % 100 == 0:
		...
		if self.observe:
			self.env.close()
		end_time = time.time()
		print '%s consumes %i tries' % ('off-policy', self.n_try)
		self.log.append('%s consumes %i tries' % ('off-policy', self.n_try))
		print '%s consumes %.2f seconds' % ('off-policy', end_time-start_time)
		self.log.append('%s consumes %.2f seconds' % ('off-policy', end_time-start_time))
		# save trajectory
	...
	def _add_state(self, state):
		...
		if state_string not in self.states_dict:
			...
			self.behavior_policy.append([1.0/len(self.actions) for _ in range(len(self.actions))])
	...
	def _estimate_state_action_function(self, trajectory):
		k = len(trajectory)-1
		while k >= 0:
			if self.policy[trajectory[k][0]][trajectory[k][1]] != 1:
				break
			k -= 1
		for trajid, info in enumerate(trajectory[k+1:]):
			...
			prod_prob = reduce(lambda x, y: x*y, \
							[self.behavior_policy[trajectory[trajid:][idx][0]][trajectory[trajid:][idx][1]] \
							for idx in range(len(trajectory[trajid:]))])
			self.state_action_count[stateid][actionid] += 1
			self.state_action_func[stateid][actionid] += 1.0 * (accumulated_reward/prod_prob - \
	...
	def _get_log_path(self, map_size, index):
		return '../experiments/monte_carlo_control/trajectory/off-policy' + \
...
for size in [15]: # map size must be odd
	...
	for i in range(20):
		...
		off_policy = MonteCarloControlOffPolicy(observe=False, save=False, \
