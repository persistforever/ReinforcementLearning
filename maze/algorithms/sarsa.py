...
class SARSA(TemporalDifference):
	...
	def train(self, iteration=1000000):
		...
		for _ in xrange(iteration):
			...
			while gone:
				sample = [stateid]
				if stateid not in trajectory_dict:
					trajectory_dict[stateid] = None
				...
				if (gone and new_stateid in trajectory_dict) or reward == -10:
					wall_string = str(stateid)
					wall_string += '#' + str(actionid)
				...
				self._estimate_state_action_function(stateid=stateid, actionid=actionid, \
													reward=reward, new_stateid=new_stateid, \
													new_actionid=new_actionid, gone=True)
				self.policy = self._soft_policy_iteration()
				# add trajectory
				sample.append(reward)
				trajectory.append(sample)
	...
	def _get_log_path(self, map_size, index):
		return '../experiments/temporal_difference/trajectory/SARSA_(' + \
...
for size in [15]: # map size must be odd
	...
	for i in range(20):
		...
		sarsa = SARSA(observe=False, save=False, map_size=(size,size), index=i, seed=i)
		sarsa.train()