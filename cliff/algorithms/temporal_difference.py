...
from environment.cliff import Environment


class TemporalDifference:

	def __init__(self, observe=True, save=False, map_size=(10,4), index=0, seed=0):
	...
	def _can_exit(self, rewards):
		n_win = sum([1 for reward in rewards if reward == 100])
		return True if n_win > 35 else False
	...
	def _save_trajectory(self, trajectory_list, win_trajectory):
		self.log.append('%s reward per trajectory %s' % ('-'*20, '-'*20))
		for trajectory in trajectory_list:
			reward_sum = 0.0
			for idx, [stateid, _, reward] in enumerate(trajectory):
				reward_sum += (self.gamma ** idx) * reward
			self.log.append(str(reward_sum))
