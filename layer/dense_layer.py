...
class DenseLayer:
    ...
    def __init__(self, input_shape, hidden_dim, activation='relu', dropout=False, 
        ...
        # 权重矩阵
        scale = math.sqrt(2.0 / (self.input_shape[1]))
        init_value = scale * numpy.random.normal(size=[
            self.input_shape[1], self.hidden_dim], loc=0.0, scale=0.01)
        self.weight = tf.Variable(init_value, dtype=tf.float32, name='W_%s' % (name))
